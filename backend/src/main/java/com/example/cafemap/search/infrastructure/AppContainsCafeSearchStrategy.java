...
@Component
public class AppContainsCafeSearchStrategy extends AbstractCafeSearchStrategy implements CafeSearchStrategy {

    public AppContainsCafeSearchStrategy(SearchCafeRepository searchCafeRepository) {
        super(searchCafeRepository);
    }

    @Override
    public SearchStrategyType type() {
        return SearchStrategyType.APP_CONTAINS;
    }

    @Override
    public List<CafeSearchItem> search(CafeSearchCommand command) {
        return appCandidates(command).stream()
                .filter(row -> matchesAny(row, command.normalizedQuery()))
